...
impl <'a>Widget for ErrorBox<'a> {
    fn draw(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        buf.set_string(error_text_start_position, message_rect.y + message_rect.height / 2 - 2, error_text, fill_style);
        ...
        for line in controls_text.iter() {
            if text_max_length >= line.chars().count() as u16 {
                ...
                buf.set_string(start_position, message_rect.y + message_rect.height / 2 + 2, line, fill_style);
                break;
            }
        }
    }
